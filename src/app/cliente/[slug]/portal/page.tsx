...
export default function PortalPremium() {
  ...
  useEffect(() => {
    async function carregarConteudosDoServidor() {
      ...
      try {
        ...
        const resposta = await fetch(
          `/api/cliente/direcionamentos?slug=${encodeURIComponent(
            slug
          )}`,
          {
            cache: "no-store",
        ...
        const conteudos: ConteudoPlanilha[] =
          (
            Array.isArray(resultado?.conteudos)
              ? resultado.conteudos
              : []
          ).map((item: any) => ({
            slug: String(
